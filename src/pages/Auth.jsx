...
export default function Auth() {
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      alert("Simulated: Continue with Google clicked!");
    } catch (err) {
      ...
      <div className="w-full max-w-[440px] relative z-10 flex flex-col gap-6">
        <div className="bg-surface-container-lowest rounded-2xl shadow-xl overflow-hidden border border-outline-variant/30 hover:shadow-2xl transition-all duration-300">
          ...
          <div className="p-8">
            ...
            {/* Errors Panel */}
            {error && (
              <div className="mb-6 p-3 bg-error-container text-on-error-container rounded-xl text-body-sm flex items-center gap-2">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              {!isLogin && (
                <div className="relative group">
                  <label className="absolute -top-2 left-3 px-1 bg-surface-container-lowest text-[10px] uppercase font-semibold tracking-wider text-on-surface-variant z-10 group-focus-within:text-primary transition-colors">
                    Full Name
                  </label>
                  <div className="relative flex items-center">
                    <span className="material-symbols-outlined absolute left-3 text-on-surface-variant group-focus-within:text-primary transition-colors text-[20px]">person</span>
                    <input 
                      className="w-full bg-surface-container-lowest text-on-surface text-body-md rounded-xl py-3 pl-10 pr-4 outline-none ring-1 ring-outline-variant focus:ring-2 focus:ring-primary focus:border-transparent transition-all shadow-sm placeholder:text-on-surface-variant/40"
              ...
              <div className="relative group">
                ...
                <div className="relative flex items-center">
                  <span className="material-symbols-outlined absolute left-3 text-on-surface-variant group-focus-within:text-primary transition-colors text-[20px]">mail</span>
                  <input 
                    className="w-full bg-surface-container-lowest text-on-surface text-body-md rounded-xl py-3 pl-10 pr-4 outline-none ring-1 ring-outline-variant focus:ring-2 focus:ring-primary focus:border-transparent transition-all shadow-sm placeholder:text-on-surface-variant/40"
              ...
              <div className="relative group">
                ...
                <div className="relative flex items-center">
                  <span className="material-symbols-outlined absolute left-3 text-on-surface-variant group-focus-within:text-primary transition-colors text-[20px]">lock</span>
                  <input 
                    className="w-full bg-surface-container-lowest text-on-surface text-body-md rounded-xl py-3 pl-10 pr-10 outline-none ring-1 ring-outline-variant focus:ring-2 focus:ring-primary focus:border-transparent transition-all shadow-sm placeholder:text-on-surface-variant/40"
                    placeholder="••••••••" 
                    value={password}
                  ...
                  />
                  <button 
                    className="absolute right-3 text-on-surface-variant hover:text-primary transition-colors flex items-center justify-center p-1"
              ...
              {isLogin && (
                <div className="flex items-center justify-between text-xs mt-1">
                  <label className="flex items-center gap-2 cursor-pointer group">
                    <input 
                      type="checkbox" 
                      className="w-4 h-4 accent-primary rounded border-outline-variant cursor-pointer"
                    />
                    <span className="text-on-surface-variant group-hover:text-on-surface transition-colors">Remember me</span>
                  </label>
                  <button 
                    type="button" 
                    onClick={() => alert("Simulated: Forgot password requested.")}
                    className="text-primary hover:text-primary-container font-semibold transition-colors hover:underline"
                  >
                    Forgot password?
                  </button>
                </div>
              ...
              <button 
                className="w-full bg-primary text-on-primary text-title-md font-bold py-3 rounded-xl shadow-md hover:shadow-lg hover:bg-primary-container hover:text-on-primary-container transition-all relative overflow-hidden group flex items-center justify-center gap-2"
                type="submit"
                disabled={submitting}
              >
                {submitting ? (
        ...
        <div className="bg-surface-container-lowest border border-outline-variant rounded-2xl shadow-md p-4 text-xs space-y-2">
          ...
          <div className="grid grid-cols-3 gap-2 pt-1">
            <button 
              onClick={() => autofill('owner@example.com')}
            ...
            <button 
              onClick={() => autofill('editor@example.com')}
            ...
            <button 
              onClick={() => autofill('viewer@example.com')}
